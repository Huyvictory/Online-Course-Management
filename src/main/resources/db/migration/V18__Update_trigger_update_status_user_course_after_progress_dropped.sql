...
BEGIN
    ...
    DECLARE dropped_lessons INT;
    DECLARE course_status VARCHAR(20);
    DECLARE current_course_status VARCHAR(20);

    -- First check if the course status
    SELECT status INTO current_course_status
    FROM user_courses
    WHERE user_id = NEW.user_id
      AND course_id = NEW.course_id;

    -- Get all lesson counts including dropped
    SELECT
        COUNT(CASE WHEN ulp.status = 'COMPLETED' THEN 1 END),
        COUNT(CASE WHEN ulp.status = 'IN_PROGRESS' THEN 1 END),
        COUNT(CASE WHEN ulp.status = 'DROPPED' THEN 1 END),
        COUNT(DISTINCT l.id)
    INTO completed_lessons, in_progress_lessons, dropped_lessons, total_lessons
    ...
    IF dropped_lessons = total_lessons THEN
        SET course_status = 'DROPPED';
    ELSEIF completed_lessons = total_lessons THEN
